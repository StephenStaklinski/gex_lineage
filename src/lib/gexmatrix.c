...
static int gene_has_case_insensitive_prefix(const char *gene_name,
                                            const char *prefix) {
    int i;

    if (gene_name == NULL || prefix == NULL)
        return 0;

    for (i = 0; prefix[i] != '\0'; i++) {
        if (gene_name[i] == '\0')
            return 0;
        if (toupper((unsigned char)gene_name[i]) !=
...
static int is_ribo_mito_gene(const char *gene_name) {
    static const char *prefixes[] = {
        "RPL", "RPS", "RPLP", "MRPL", "MRPS", "MT"
    };
    int i;

    for (i = 0; i < (int)(sizeof(prefixes) / sizeof(prefixes[0])); i++) {
        if (gene_has_case_insensitive_prefix(gene_name, prefixes[i]))
            return 1;
    }
...
int gex_remove_ribo_mito_genes(GexMatrix *gex) {
    int read_col, write_col, i;
    int n_genes;

    if (gex == NULL || gex->X == NULL || gex->gene_names == NULL)
        return -1;

    n_genes = gex->X->ncols;
    write_col = 0;

    for (read_col = 0; read_col < n_genes; read_col++) {
        if (is_ribo_mito_gene(gex->gene_names[read_col])) {
            free(gex->gene_names[read_col]);
            gex->gene_names[read_col] = NULL;
            continue;
        }

        if (write_col != read_col) {
            for (i = 0; i < gex->X->nrows; i++)
                mat_set(gex->X, i, write_col, mat_get(gex->X, i, read_col));
            gex->gene_names[write_col] = gex->gene_names[read_col];
            gex->gene_names[read_col] = NULL;
        }
        write_col++;
    }
    ...
    mat_resize(gex->X, gex->X->nrows, write_col);
    return n_genes - write_col;
}
